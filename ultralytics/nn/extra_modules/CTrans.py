...
class Reconstruct(nn.Module):
    ...
    def forward(self, x, target_size):
        ...
        # 使用目标尺寸进行上采样，而非固定缩放因子
        x = nn.Upsample(size=target_size)(x)
...
class ChannelTransformer(nn.Module):
    def __init__(self, channel_num=[64, 128, 256, 512], img_size=640, vis=False, patchSize=[40, 20, 10, 5]):
        ...
        self.reconstruct_4 = Reconstruct(channel_num[3], channel_num[3], kernel_size=1) if len(channel_num) == 4 else nn.Identity()
    ...
    def forward(self, en):
        ...
        # 使用en的尺寸作为目标尺寸进行上采样
        x1 = self.reconstruct_1(encoded1, en1.shape[2:]) if en1 is not None else None
        x2 = self.reconstruct_2(encoded2, en2.shape[2:]) if en2 is not None else None
        x3 = self.reconstruct_3(encoded3, en3.shape[2:]) if en3 is not None else None
        x4 = self.reconstruct_4(encoded4, en4.shape[2:]) if en4 is not None else None
