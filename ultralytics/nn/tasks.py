...
from ultralytics.nn.extra_modules import CBAM  # 新增导入语句
try:
    import thop
except ImportError:
    thop = None
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if isinstance(c2, list) and m not in {ChannelTransformer, PyramidContextExtraction, CrossLayerChannelAttention, CrossLayerSpatialAttention, MutilScaleEdgeInfoGenetator}:
            ...
            # Handle Warehouse_Manager parameter conversion
            if hasattr(m, '__name__') and m.__name__ in ['KWConv', 'C3k_KW', 'C3k2_KW', 'Bottleneck_KW']:
                # Check if warehouse_manager is a boolean and convert to Warehouse_Manager instance
                if len(args) >= 4 and isinstance(args[3], bool) and args[3] is True:
                    from ultralytics.nn.extra_modules.kernel_warehouse import Warehouse_Manager
                    args = list(args)
                    args[3] = Warehouse_Manager()
                    args = tuple(args)
